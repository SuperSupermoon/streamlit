...
if st.session_state.reviewer_name:
    ...
    for sec, content in sections.items():
        ...
        for i in range(st.session_state.row_count_state[sec]):
            st.text_area(f"Additional feedback for {sec} section", key=f"{sec}_feedback_for_row_{i+1}")

        
        # # 'Submit Feedback' 버튼
        if col_add_remove_submit[2].button('Submit (when there are sth to add.)', key=f"additional_submit_button_{sec}"):
